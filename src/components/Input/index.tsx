...
const FiltersInput: FC<IFilterInput> = ({
  setFiltersPriceFrom,
  setFiltersPriceTo,
  setFiltersAgeFrom,
  setFiltersAgeTo,
  ...
  const price = useTypedSelector(
    (selector) => selector.filtersSlice.flats.price[type || "from"]
  );
  const age = useTypedSelector(
    (selector) => selector.filtersSlice.neighbors.age[type || "from"]
  );
  const [value, setValue] = useState<string>(
    String(page === "flats" ? price : age)
  );
    ...
    <div className={styles.input}>
      <input
        ...
        onChange={(e) => {
          ...
          if (parseInt(value) > (page === "flats" ? maxFlatPrice : maxUsersAge))
            value = String(page === "flats" ? maxFlatPrice : maxUsersAge);
          setValue(String(parseInt(value)));

          if (page === "flats") {
            if (type === "from")
              setFiltersPriceFrom && setFiltersPriceFrom(Number(value));
            else setFiltersPriceTo && setFiltersPriceTo(Number(value));
          } else if (page === "neighbors") {
            if (type === "from")
              setFiltersAgeFrom && setFiltersAgeFrom(Number(value));
