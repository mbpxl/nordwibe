...
const FavoritesPage: React.FC = () => {
  ...
  const isOnFavoritesUsersPage = pathname === '/favourites/users';
    ...
    <div className={styles.favoritePage}>
      ...
      <nav className={styles.nav_fav}>
        <Link href="/favourites">
          <h2 className={isOnFavoritesPage ? styles.activeLink : styles.inActiveLink}>Избранные квартиры</h2>
        </Link>
        <Link href="/favourites/users">
          <h2 className={isOnFavoritesUsersPage ? styles.activeLink : styles.inActiveLink}>Избранные пользователи</h2>
        </Link>
      </nav>
        ...
        <div className={styles.favourites}>
          {favoriteHouses.map((flat) => (
            ...
            <div key={flat.id} className={styles.flat}>
              ...
              <div className={styles.containerUser}>
                <div className={styles.userCard}>
                  ...
                  <div className={styles.userInformation}>
                    <h1>{usersList[flat.creator_id]?.first_name || 'Пользователь'}</h1>
                    <h4>{usersList[flat.creator_id]?.my_town || 'Не указан город'}</h4>
                  </div>
                </div>
                <h3>{flat.cost}%</h3>
              </div>
              ...
              {/* Коммунальные услуги */}
              <span className={styles.commun}>Коммуналка: {flat.cost_utilities} руб/мес</span>
              ...
              <div className={styles.flatInfo}>
                <div className={styles.blocks}>
                  <ul>
                    ...
                    <li>{flat.i_am_owner ? 'Я собственник' : 'Снимаю эту квартиру'}</li>
                    <li>{flat.is_possible_animals ? 'Можно с животными' : 'Без животных'}</li>
            ...
            <div className="">
              <button onClick={() => onRemoveFlatFromFavotite(Number(flat.id + 1))}>
                Remove
              </button>
              <ToastContainer/>
            </div>
